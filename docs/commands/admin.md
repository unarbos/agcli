...
For `admin raw --call proposals --args '[]'`, `--output json` prints:
...
| `raw` | `--call <String>` `--args <JSON array>` `--sudo-key <String?>` | `AdminUtils::<dynamic call name>` for standard calls; governance storage reads for `--call proposals` alias | JSON numbers become `u128`, bools become bool, strings become string | Depends on call; `proposals` reads `<governance pallet>::Proposals`, `ProposalOf`, `Voting` (tries `Triumvirate`, falls back to `Senate`) | `Sudo::Sudid(...)` for runtime dispatch result; no extrinsic for `proposals` alias |
| `list` | no args | local only, does not submit to chain | n/a | n/a | n/a |

## `admin raw` accepted call names

`admin raw` is restricted by `validate_admin_call_name` to agcli's local `known_params` list, not the full runtime call set, except for the governance read alias `--call proposals` (expects `--args '[]'`).

Current allowed admin call names are:
...
agcli --network local --output json admin raw --call proposals --args '[]'
