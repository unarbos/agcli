...
pub async fn live_dynamic(client: &Client, interval_secs: u64) -> Result<()> {
    let interval = Duration::from_secs(if interval_secs == 0 { DEFAULT_POLL_SECS } else { interval_secs });
    ...
    loop {
        ...
        if !deltas.is_empty() {
            ...
            for d in &deltas {
                let arrow = if d.price_pct > 0.0 { "↑" } else if d.price_pct < 0.0 { "↓" } else { "→" };
                println!(
...
pub async fn live_metagraph(client: &Client, netuid: NetUid, interval_secs: u64) -> Result<()> {
    ...
    println!("Live metagraph for SN{} (polling every {}s, Ctrl+C to stop)\n", netuid.0, interval.as_secs());
    ...
    loop {
        ...
        for c in &curr_neurons {
            if let Some(p) = prev_map.get(&c.uid) {
                ...
                changes.push(format!("  UID {:<4} NEW neuron (hotkey: {})", c.uid, &c.hotkey[..8]));
            }
        }
...
pub async fn live_portfolio(client: &Client, coldkey_ss58: &str, interval_secs: u64) -> Result<()> {
    ...
    println!("Live portfolio for {} (polling every {}s, Ctrl+C to stop)\n", coldkey_ss58, interval.as_secs());
    println!("Free: {}  Staked: {}  Positions: {}", prev.free_balance, prev.total_staked, prev.positions.len());
